...
from achilles_api.session import SessionNotFoundError, store
from achilles_api.upload_jobs import UploadJobNotFoundError, upload_jobs
from achilles_api.waitlist import SlidingWindowRateLimiter, WaitlistStore, is_valid_email
...
app = FastAPI(
    title="Achilles API",
    version="0.1.0",
    description="Backend for the Achilles organizational stress-testing platform.",
)
...
def _serialize_state(state: Any) -> dict[str, Any]:
    from achilles_api.models.graph import State
...
async def get_standard(session_id: str) -> JSONResponse:
    ...
    from achilles_api.ingestion.standard import load_standard
...
async def reingest(body: ReingestRequest) -> JSONResponse:
    ...
    from achilles_api.ingestion.extractor import detect_gaps
    from achilles_api.ingestion.standard import build_graph_from_standard, load_standard
...
async def explore(body: ExploreRequest) -> JSONResponse:
    ...
    from achilles_api.engine.state_tree import ExplorationConfig, build_state_tree, tree_stats
    from achilles_api.results.ranking import build_final_report
    ...
    if mc_config is not None:
        from achilles_api.mc.resilience import run_resilience_analysis
    ...
    try:
        from achilles_api.results.ranking import generate_all_narratives
...
async def chat(body: ChatRequest) -> JSONResponse:
    ...
    from achilles_api.gemini import (
        generate_content_stream_with_fallback,
        generate_content_with_fallback,
    )
    from achilles_api.session import ChatMessage
...
async def ws_chat(websocket: WebSocket, session_id: str) -> None:
    ...
    from achilles_api.chat.knowledge import EXECUTIVE_SYSTEM_PROMPT, build_knowledge_context
    from achilles_api.gemini import generate_content_stream_with_fallback
    from achilles_api.session import ChatMessage
