...
from achilles_api.ingestion.scoring import (
    clamp,
    score_edge_weight,
    score_recovery,
    score_theta,
...
def build_context(parsed_files: list[ParsedFile], description: str = "") -> str:
    ...
    ----------
    parsed_files:
        Output from :func:`achilles_api.ingestion.parser.parse_files`.
    description:
...
def build_graph_from_dict(
    ...
    *,
    ...
    if refine_inference:
        try:
            from achilles_api.engine.weight_inference import refine_weights
...
async def _ingest_impl(
    ...
    **Fast path** — If any uploaded ``.json`` file is already in the Achilles
    ...
    **Slow path** — Parse all files, call Gemini for graph extraction,
    ...
    IngestResult
    """
    from achilles_api.ingestion.parser import parse_files
